...
def _csv(path: str, results: list[dict[str, Any]]) -> None:
    fields = [
        ...
        "risk_scores", "signals", "consensus", "conflicts", "confidence", "ai_assessment", "evidence",
    ]
    with Path(path).open("w", encoding="utf-8-sig", newline="") as handle:
        ...
        for result in results:
            ...
            for key in ("risk_scores", "signals", "consensus", "conflicts", "confidence", "ai_assessment", "evidence"):
...
def _scan(args: argparse.Namespace) -> int:
    ...
    for result in results:
        result.ai_assessment = build_ai_assessment(result.country_code, proxy=result.proxy, vpn=result.vpn, tor=result.tor, datacenter=result.datacenter, risk_scores=result.risk_scores)
...
def _subscription(args: argparse.Namespace) -> int:
    ...
    if args.json:
        ...
        parse = data["parse"]
        print(f"Nodes: {parse['node_count']} | protocols: {parse['protocols']} | directly exposed public IPs: {len(data['resolution']['direct_exposed_public_ips'])}")
        print(f"Domain-only nodes: {data['resolution']['domain_only_node_count']} | actual traffic exits observable from subscription: 0")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="ipbatch",
        description="Evidence-first IP investigation with read-only subscription parsing and no VPN/proxy connection capability",
    )
    ...
    formats.set_defaults(func=lambda args: (print(
        ...
        "fsl64/fslyaml passthrough. Only public IP literals directly exposed in server fields are investigated; domain DNS answers are observations only."
    ) or 0))
