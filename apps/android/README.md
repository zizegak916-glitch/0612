...
Build with `./build.sh`. The script downloads toolchain files into `.toolchain/`, runs parser/downloader smoke tests and creates `build/IPBatchInspector-v4.1.0-android-debug.apk` with a local debug signature. Do not publish that debug key or represent this artifact as a store-signed release.
